//! MVCモデルのControlerの役割
//! ユーザーからのリクエストを処理し、適切なモデルとビューを決定。主にビジネスロジックを担当
//! 本（book）に関連するリクエストを処理する関数（getAllBooks, getBookById, registerBook, updateBook, deleteBook）が定義
//! これらの関数がModelとViewの間の橋渡しをする役割を果たしています。
//! Mongooseを使用してMongoDBとのやり取りを行っています。また、express-validatorを使用してリクエストのバリデーションも行っています
